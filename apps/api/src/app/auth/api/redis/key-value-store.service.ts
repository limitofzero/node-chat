...
export class KeyValueStoreService {
  ...
  constructor(
    @Inject(REDIS) private readonly redis: IORedis.Redis
  ) {
  ...
  public remove(key: string): Observable<void> {
    return defer(() => this.redis.del(key)).pipe(
      mapTo(null)
    );
  }
  ...
  public getAndRemove<T>(key: string): Observable<T> {
    return this.get<T>(key).pipe(
      mergeMap(value => this.remove(key).pipe(
        mapTo(value)
      ))
    );
  }
  ...
  public set<T>(key: string, value: T): Observable<void> {
    ...
    return defer(() => this.redis.set(key, valueAsString)).pipe(
      mapTo(null)
    );
  }
